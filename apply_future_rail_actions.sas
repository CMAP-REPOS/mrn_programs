...
%macro changes;
    ...
    *** -- APPLY ACTION CODE 3 (NEW STATION) -- ***;
    %if &a3>0 %then %do;
        ...
        data itins; set itins;
            ...
            ln=substr(tr_line,1,3);
        proc sort; by ln itin_a itin_b;
        data itins; merge itins(in=hit) itn3; by ln itin_a itin_b; if hit;
        ...
        ** -- Insert New Station into Itinerary -- **;
        data fix; set fix;
           id=it_order; output;
           id=it_order+0.1; output;
        data fix; set fix;
          if id=int(id) then do; itin_b=new; layover='0'; l=0; zn_fare=0; end;
          else do; itin_a=new; end;
          trv_time=round(trv_time/2+0.5,0.1);   ***add 0.5 minutes on each side of new station;

        data itins; set itins fix;  proc sort; by tr_line it_order id;
        data itins(drop=it_order new id ln); set itins; group=lag(tr_line);
        data itins; set itins; 
            retain it_order 1;
            it_order+1;
    ...
    *** -- APPLY ACTION CODE 7 (NEW CONSOLIDATED STATION) -- ***;
    %if &a7>0 %then %do;
	    ...
	    else if start>0 and d=1 then do; start=.; end=.; new=.; end;

        data itins (drop=f n e t); set itins;
            retain f 0 n e t 0;
	    if itin_a=start then do; f=1; n=new; e=end; end;
        ...
        data itins (drop=time t); merge itins(in=hit) time; by tr_line it_order; if hit;    *** adjust travel time for new itin;
  ...
  %if &a4>0 %then %do;
       ...
       %do %while (&count le &total);                                                                 ** loop through future routes coded;                                       
           ...
           data itn4(drop=f_meas t_meas); merge ftritin limit(in=hit); by tr_line; if hit; 
           ...
           data check4; set itn4(where=(it_order>-1 and it_order<1001));    **check that it_order is valid;
               proc print; title 'BAD ITINERARY ORDER FOR ACTION=4';

           data itn4; set itn4; ord=lag(it_order);
           data itn4; set itn4; 
             retain group 0;
           ...
           data part; set part;
             if group=1 then node=itin_b; else if group=2 then node=itin_a; 
              proc summary nway; class ln; var node; output out=junk min=node;

           data nwitn; set itins; length ln $3.; ln=substr(tr_line,1,3);
           data adj(drop=_type_ _freq_); merge nwitn(in=hit1) junk(in=hit2); by ln; if hit1 & hit2;
              proc sort; by tr_line it_order;
           data adj; set adj; by tr_line it_order;
             if first.tr_line  & itin_a=node then beg=1; else beg=0;
             if last.tr_line & itin_b=node then end=1; else end=0;
            proc summary nway; class tr_line; id ln; var beg end; output out=cntl max=;
           data cntl(drop=_type_ _freq_); set cntl; num=_n_;
           %let cntl=0; %let cnter=1;
           data _null_; set cntl nobs=totobs; call symput('cntl',left(put(totobs,8.))); run;        ** store number of control lines;

             ** -- Loop through Control Routes and add extension to beginning/end as appropriate --**;
              %do %while (&cnter le &cntl);   
                 %let start=0; %let fin=0; %let rt=xxxxxx; %let ln3=xxx;
                 data temp; set cntl(where=(num=&cnter));
                   call symput('rt',left(put(tr_line,$6.))); call symput('start',left(put(beg,1.))); 
                   call symput('fin',left(put(end,1.))); call symput('ln3',left(put(ln,$3.))); run;

                 data itn4a itn4b; set itins;
                   if tr_line="&rt" then output itn4a; else output itn4b;
                 ...
                 %if &fin=1 %then %do;                                                                ** add extension to end of line;
                     data itn4a(drop=ln ord group); set itn4a itn4(where=(ln="&ln3" & it_order>1000));
                       if count(tr_line,'*')>0 then tr_line="&rt";
                       if l>0 & it_order<1000 then do; l=0; layover='0'; end;
                 %end;
 
                 data itins; set itn4a itn4b; proc sort; by tr_line it_order;
                %let cnter=%eval(&cnter+1);
              %end;  
         %let count=%eval(&count+1);
       %end;

       data itins(drop=it_order); set itins; group=lag(tr_line);
       data itins(drop=group); set itins; 
        retain it_order 1;
         it_order+1;
         if tr_line ne group then it_order=1;
  ...
  data routes; set routes; length ln $3.; ln=substr(tr_line,1,3);
