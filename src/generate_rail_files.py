...
for scenario in scen_list:
    ...
    temp_route_ftr_shp = os.path.join(tempdir, 'temp_route_ftr.shp')
    temp_arc_shp = os.path.join(tempdir, 'temp_arc.shp')
    temp_arc_ftr_shp = os.path.join(tempdir, 'temp_arc_ftr.shp')
    temp_ppl_mvr_dbf = os.path.join(tempdir, 'temp_ppl_mvr.dbf')
    ...
    scen_itin_dbf = os.path.join(tempdir, 'scen_itin.dbf')
    ftr_itin_dbf = os.path.join(tempdir, 'ftr_itin.dbf')
    zones = os.path.realpath(os.path.join(srcdir, '../input/zones17/zones17.shp'))
    temp_rlnode_zone_shp = os.path.join(tempdir, 'temp_rlnode_zone.shp')
    outFl = os.path.join(tempdir, 'rte_out.txt')
    ...
    if not os.path.exists(tempdir):
        arcpy.AddMessage("---> Directory created: " + tempdir)
        os.mkdir(tempdir)
    ...
    arcpy.FeatureClassToFeatureClass_conversion(railnet_arc, tempdir, "temp_arc.shp", "", "", "")
    ...
    arcpy.FeatureClassToFeatureClass_conversion(rail_routes_ftr, tempdir, "temp_route_ftr.shp", "", "", "")
    arcpy.SelectLayerByLocation_management(railnet_arc, "SHARE_A_LINE_SEGMENT_WITH", rail_routes_ftr, "", "NEW_SELECTION")
    arcpy.FeatureClassToFeatureClass_conversion(railnet_arc, tempdir, "temp_arc_ftr.shp", "", "", "")
