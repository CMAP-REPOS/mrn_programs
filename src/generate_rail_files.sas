...
proc import datafile="&inpath.\temp_route.dbf" out=routes1 replace;

* READ IN RAIL ITINERARY INFORMATION;
proc import datafile="&inpath.\scen_itin.dbf" out=itins1 replace;
...
proc import datafile="&inpath.\temp_rlnode_zone.dbf" out=nodes replace;

data nodes(keep=node label pspace pcost ftr_pspace ftr_pcost point_x point_y zone); set nodes;
    rename zone17=zone;

* READ IN LINK INFORMATION;
proc import datafile="&inpath.\temp_arc.dbf" out=network replace;

* READ IN PEOPLE MOVER INFORMATION AND REMOVE APPROPRIATE LINKS;
proc import datafile="&inpath.\temp_ppl_mvr.dbf" out=people_mover replace;
...
%macro future;
    ...
    */
    /*%if &sc>&basescen %then %do;*/
        ...
        proc freq data=ftrrte noprint;
            tables actcode / out=ftract;

        data _null_; set ftract;
            call symputx(actcode,count);  * Store count of action codes as macro variables;

        proc import datafile="&inpath.\ftr_itin.dbf" out=ftritin replace;  * Corresponding future itineraries;

        proc sort data=ftritin; by tr_line;

        data temprte(keep=tr_line); set ftrrte;

        data ftritin; merge ftritin temprte(in=hit); by tr_line;
            if hit;

        proc import datafile="&inpath.\temp_arc_ftr.dbf" out=ftrarc replace;  * Future links;

        data network; set network ftrarc;  * Add future links to base links;
...
%macro getdata;
    ...
    */
    ...
    %do %while (&counter le 4);

        * Set time-of-day selection criteria;
        %if &counter=1 %then %let select=strthour ge 18 or strthour lt 6;                      
        %else %if &counter=2 %then %let select=strthour ge 6 & strthour lt 9;
        %else %if &counter=3 %then %let select=strthour ge 9 & strthour lt 16;
        %else %if &counter=4 %then %let select=strthour ge 16 & strthour lt 18;
        
        %let tod=&counter;  * Set time-of-day value;
        ...
        * Output files;
        ...
        data routes(rename=(descriptio=descr)); set routes1(where=(&select));
            it_order=0;
            acthdwy=.;
        ...
        %if &a1>0 %then %do;
            ...
            * Include full future routes (action=1) in processing;
            data frte2(drop=actcode headway c rename=(phdwy=headway)); set ftrrte(where=(action=1 and (tod ? "&tod")));
                c=count(headway,':');
                if c>0 then c=c+1;
                if c>0 then do;
                    do i=1 to c by 2;
            ...
            data itins1; set itins1 ftritin;
                proc sort nodupkey; by tr_line it_order;
            
            %end;

        proc freq data=routes;
            tables tr_line / noprint out=keep;

        data itins(drop=f_meas t_meas count percent); merge itins1(in=hit1) keep(in=hit2); by tr_line;
            if hit1 & hit2;
        ...
        * If applicable, process future route action codes;
        %if &a2>0 or &a3>0 or &a4>0 or &a5>0 or &a7>0 %then %do;

            %include "&srcpath.\apply_future_rail_actions.sas";
        ...
        * Report layover problems.;
        ...
        proc freq;
            tables tr_line / noprint out=check;
        ...
        * Report itinerary gaps.;
        ...
        proc print;
            var tr_line itin_a itin_b it_order z;
        ...
        * Collapse CTA rail into generalized service.;
        ...
        %include "&srcpath.\collapse_CTA_runs.sas";

        * Update runs.;
        data rtem rtec; set routes;
            if mode='M' then output rtem;
            else output rtec;

        proc sort data=rtec; by tr_line;

        data rtec(drop=hdwy group runs); merge rtec keepc(in=hit); by tr_line;
            if hit;
            if acthdwy>0 then headway=acthdwy;
            else headway=hdwy;
        ...
        data rtem; set rtem;
            if headway=99 then do;
                ...
                /*
                *** Old transit TODs (C21Q2 and earlier);
                if &tod=1 then headway=600;
                else if &tod=2 or &tod=4 then headway=60;
                else if &tod=5 then headway=240;
                */

                if &tod=1 then headway=720;
                else if &tod=2 then headway=180;
                else if &tod=3 then headway=420;
                else headway=120;
                end;
        ...
        data routes; set rtec rtem;
            ...
            ds=substr(descr,1,20);
            proc sort; by tr_line;

        data rte(drop=it_order); set routes;

        data itins; merge itins rte(in=hit); by tr_line;
            if hit;
            proc sort; by tr_line it_order;
        ...
        data combine; set combine; by tr_line;
            ...
            if _n_=1 then do;
                put "c RAIL TRANSIT BATCHIN FILE FOR SCENARIO &scen.00 TOD &tod" /
                    "c &sysdate" / "c us1 holds segment travel time, us2 holds zone fare" /
                    "t lines init";
                end;
            if first.tr_line then do;
                put 'a' +2 name +2 mode +2 veh_type +2 headway +2 speed +2 desc /
                    +4 'path=no';
                end;
            else if (it_order=1 & last.tr_line) then do;
                put +4 'dwt=0.01' /
                    +4 itin_a +(7-length(left(trim(itin_a)))) 'ttf=1' +2 'us1=' +0 trv_time +(6-length(left(trim(trv_time)))) 'us2=' +0 zn_fare /
                    +4 itin_b +(7-length(left(trim(itin_b)))) 'lay=' +0 layover;
                end;
            else if it_order=1 then do;
                put +4 'dwt=0.01' /
                    +4 itin_a +(7-length(left(trim(itin_a)))) d +(10-length(left(trim(d)))) 'ttf=1' +2 'us1=' +0 trv_time +(6-length(left(trim(trv_time)))) 'us2=' +0 zn_fare;
                end;
            else if last.tr_line then do;
            ...
            else if (layover ne '0' and layover ne '') then do;
                put +4 itin_a +(7-length(left(trim(itin_a)))) d +(10-length(left(trim(d)))) 'ttf=1' +2 'us1=' +0 trv_time +(6-length(left(trim(trv_time)))) 'us2=' +0 zn_fare +2 'lay=' +0 layover;
                end;
            else do;
                put +4 itin_a +(7-length(left(trim(itin_a)))) d +(10-length(left(trim(d)))) 'ttf=1' +2 'us1=' +0 trv_time +(6-length(left(trim(trv_time)))) 'us2=' +0 zn_fare;
                end;
        ...
        * PART 1. LIMIT LINKS TO THOSE IN ITINERARIES.;
        ...
        data net1; merge network1 used (in=hit); by anode bnode;
            if hit;  * Keep only mainline links with service;
        ...
        * PART 2. LIMIT AUXILIARY LINKS TO THOSE CONNECTED TO SERVICE.;
        ...
        proc freq;
            tables node / noprint out=test0;

       data test1(keep=anode acount); set test0;
           anode=node;
           acount=count;

       data test2(keep=bnode bcount); set test0;
           bnode=node;
           bcount=count;
           proc sort data=auxlink; by anode;

        data auxlink; merge auxlink (in=hit) test1; by anode;
            if hit;
            proc sort; by bnode;

        data auxlink; merge auxlink (in=hit) test2; by bnode;
            if hit;
        ...
        * UPDATE NODE INFORMATION.;
        ...
        proc freq;
            tables node / noprint out=netnodes;
        ...
        data nodes1; merge nodes netnodes (in=hit); by node;
            if hit;
        ...
        * VERIFY THAT EACH NODE HAS A UNIQUE NUMBER.;
        proc freq data=nodes1;
            tables node / noprint out=check;
        ...
        * WRITE OUT NETWORK BATCHIN FILE.;
        data _null_; set nodes1;
            file out3;
            if _n_= 1 then put "c RAIL NETWORK BATCHIN FILE FOR TRANSIT SCENARIO &scen.00 TOD &tod." / "c  &sysdate" /  'c a  node  x  y ui1 ui2 ui3 label' / 't nodes';
        ...
        * WRITE OUT ALL LINKS FOR LINKSHAPE FILE CREATION.;
        ...
        %if &tod=4 %then %do;
            ...
            proc export data=all_links dbms=csv outfile="&outpath.\&scen.00\rail_links_all.csv" replace;
                putnames=no;
        ...
        * CREATE SUMMARY FOR TOD PERIODS 2 & 4;
        %if &tod=2 or &tod=4 %then %do;
            ...
            * DIRECTIONAL LINK MILES.;
            ...
            data dir_lnk_mi; merge dir_links(in=hit) miles; by itin_a itin_b;
                if hit;

            proc sql;
                create table dir_lnk_mi_sum as
            ...
            * SERVICE MILES.;
            ...
            data c_itin_miles; merge c_itin(in=hit) miles; by itin_a itin_b;
                if hit;

            proc sql;
                create table c_run_miles as
            ...
            data c_srvc_miles; merge c_run_miles(in=hit) cta_runs; by tr_line;
                if hit;
                all_run_mi=run_mi*c_runs;

            proc sql;
                create table c_srvc_mi_sum as
                select mode, longname, sum(all_run_mi) as srvc_mi
                from c_srvc_miles
                group by mode, longname;

            data m_itin_miles; merge m_itin(in=hit) miles; by itin_a itin_b;
                if hit;

            proc sql;
                create table m_srvc_mi_sum as
            ...
            * SERVICE HOURS.;
            ...
            data c_run_times; merge c_times(in=hit) cta_runs; by tr_line;
                if hit;
                c_run_hrs=(c_time*c_runs)/60;

            proc sql;
                create table c_srvc_hrs_sum as
                select mode, longname, sum(c_run_hrs) as srvc_hrs
                from c_run_times
                group by mode, longname;

            proc sql;
                create table m_srvc_hrs_sum as
            ...
            * AVERAGE NUMBER OF STOPS.;
            ...
            data c_tot_stops; merge c_stops(in=hit) cta_runs; by tr_line;
                if hit;
                tot_stops=stops*c_runs;

            proc sql;
                create table c_stops_sum as
            ...
            proc export data=summaries outfile=out4 dbms=csv replace; run;
