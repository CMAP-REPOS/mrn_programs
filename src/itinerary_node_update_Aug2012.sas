...
%macro split;
  %if %sysfunc(fexist(nwmi)) %then %do;
	...
	proc sort;
	    by tr_line it_order;
	proc export data=goodout outfile="&dir.\new_segments.dbf" dbms=dbf replace;
...
%macro delete_node;
    %if %sysfunc(fexist(delndf)) %then %do;
	    ...
	    * --> read in deleted node file *;
        proc import datafile="&dir.\deleted_node.dbf" dbms=dbf out=delnd replace;
...
%macro create_future_rtes;
    %if (&code=3 & %index(&origitin,future)) %then %do;
        data rte_geo;
            infile "&dir.\rte_out.txt" missover dlm=";";
