     ...
     - SECTION 3 - Specialized processing based on the input parameter provided:
          ...
          -> 2 - Call read_rail_feed_data.sas to import & format GTFS rail coding.
                 If there are itinerary gaps:
                    * Master_Rail\mrn_programs\src\write_dictionary.sas writes a Python dictionary
                         of network links and their lengths.
                    * Master_Rail\mrn_programs\src\find_shortest_path.py determines the shortest network path.
                    * Master_Rail\mrn_programs\src\read_path_output.sas updates the itineraries as needed.
...
%let srcdir=%scan(&sysparm,1,$);        ***source code directory;
%let dir=%scan(&sysparm,2,$);           ***shapefile storage directory;
%let origitin=%scan(&sysparm,3,$);      ***original itinerary .dbf file (entire path);
%let rtefile=%scan(&sysparm,4,$);       ***name of spreadsheet for importing new routes or GTFS route file (entire path);
%let code=%scan(&sysparm,5,$);          ***choice flag for Section 3 processing;
%let segfile=%scan(&sysparm,6,$);       ***name of GTFS segment file (entire path);
...
filename in1 "&dir.\geom_out.txt";
filename out1 "&dir.\geom_in.txt";
filename out2 "&dir.\dropped_routes.txt";
...
%macro errorchk;
    %if &code ne 2 %then %do;
        ...
        proc import datafile="&dir.\temp_route.dbf" dbms=dbf out=rt replace;
...
%macro choice;
  ...
  %if &code=1 %then %do; %include "&srcdir.\read_rail_coding_spreadsheet.sas"; %end;    *** call program to import & format spreadsheet rail coding ***;
  %if &code=2 %then %do; %include "&srcdir.\read_rail_feed_data.sas"; %end;              *** call program to import & format GTFS rail coding ***;
  %if &code=3 %then %do; %include "&srcdir.\itinerary_node_update_Aug2012.sas"; %end;    *** call program to insert split-link updates into itineraries ***;
  ...
  %if not(&code=3 & %index(&origitin,future)) %then %do;
   ...
   %if &keeporig>0 %then %do;
       ** -- Re-use Route Structure for Lines Already Coded -- **;
        data exist; infile "&dir.\rte_out.txt" dlm=';' missover;
        ...
        data route(keep=line1 desc1 mode1 type1 hdwy1 speed1 fdline r_id rln dir term
            ...
            set route;
                rename newline=line1 descr=desc1 mode=mode1 type=type1 headway=hdwy1
                      speed=speed1;
        proc export data=route outfile="&dir.\rte_updt.dbf" dbms=dbf replace;
        %end;
    %else %if &code=1 | (&code=3 & %index(&origitin,future)) %then %do;  *** call block for future rail coding ***;
        ...
        data rt; set rt;
            keep line1 desc1 mode1 type1 hdwy1 speed1 tod1 scen1 action1 tipid1 rspid1
                comp1 notes1 id;
        proc export data=rt outfile="&dir.\rte_updt.dbf" dbms=dbf replace;
        %end;
