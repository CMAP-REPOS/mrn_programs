...
filename in3 "&inpath.\feed_groups.txt";
filename out5 "&inpath.\&outfl";
...
data _null_;
    command="if exist &inpath.\pypath.txt (del &inpath.\pypath.txt /Q)";
    call system(command);
    command="ftype Python.File >> &inpath.\pypath.txt";
    call system(command);

data null; infile "&inpath.\pypath.txt" length=reclen;
...
data _null_;
    command="%bquote(&runpython) &srcpath.\gtfs_collapse_routes.py &inpath.\oneline.txt &inpath.\feed_groups.txt";
    call system(command);
    command="if exist &inpath.\pypath.txt (del &inpath.\pypath.txt /Q)";
    call system(command);
