...
proc import datafile="&dir.\temp_arcstart.dbf" out=a0 replace; 
data a(keep=anode bnode point_x point_y); set a0; 

   *** READ IN ENDING NODES ***;
proc import datafile="&dir.\temp_arcend.dbf" out=b0 replace;
...
%macro split;
 %if &tot>0 %then %do;  
     ...
     proc import datafile="&dir.\temp_node.dbf" out=c replace;
     ...
     proc export data=all outfile="&dir.\new_mile.dbf" dbms=dbf replace;   
...
proc export data=final outfile="&dir.\new_node.dbf" dbms=dbf replace;
...
%macro delndchk;
	...
	%if &delndtot>0 %then %do;
        proc export data=delnd outfile="&dir.\deleted_node.dbf" dbms=dbf replace;
	%end;
%mend delndchk;
%delndchk
